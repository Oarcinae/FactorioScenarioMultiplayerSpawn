...
local function ExpandOarcEnemiesGui(player)
    ...
    if (frame) then
        ...
        frame.add{type = "label", caption="This scenario is running an enemy mod. Enemies only attack while you are online. Attacks are scaled to each player's progress and activity."}
        frame.add{type = "label", caption="Attacks are not triggered by pollution clouds."}
        frame.add{type = "label", caption="Attacks are triggered by research completed and other random timers."}
        frame.add{type = "label", caption="Each attack's size and difficulty (evo) is scaled to the target player/location."}
        frame.add{type = "label", caption="More time played, more pollution in the target area, and progressing up the tech tree will increase the biter difficulty."}

        if (player.name == "Oarc") then
